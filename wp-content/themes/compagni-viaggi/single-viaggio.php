                ...
                <article class="travel-content">
                    ...
                    <div class="travel-share-section">
                        ...
                        <div class="social-share-buttons">
                            <?php
                            $travel_url = get_permalink();
                            $travel_title = get_the_title();
                            $travel_description = wp_trim_words(get_the_excerpt(), 20);
                            ?>
                            <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($travel_url); ?>"
                               target="_blank"
                            ...
                            <a href="https://twitter.com/intent/tweet?url=<?php echo urlencode($travel_url); ?>&text=<?php echo urlencode($travel_title); ?>"
                               target="_blank"
                               rel="noopener noreferrer"
                            ...
                            <a href="https://wa.me/?text=<?php echo urlencode($travel_title . ' - ' . $travel_url); ?>"
                               target="_blank"
                               rel="noopener noreferrer"
                            ...
                            <a href="mailto:?subject=<?php echo urlencode($travel_title); ?>&body=<?php echo urlencode($travel_description . ' - ' . $travel_url); ?>"
                            ...
                            <button type="button"
                                    class="social-share-btn copy-link-btn"
                                    onclick="navigator.clipboard.writeText('<?php echo esc_js($travel_url); ?>').then(() => { this.innerHTML = '<svg width=\'24\' height=\'24\' viewBox=\'0 0 24 24\' fill=\'currentColor\'><path d=\'M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z\'/></svg> Copied!'; setTimeout(() => { this.innerHTML = '<svg width=\'24\' height=\'24\' viewBox=\'0 0 24 24\' fill=\'currentColor\'><path d=\'M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z\'/></svg> Copy Link'; }, 2000); });"
                                    title="Copy link">
                    ...
                    if ($destination || $country) :
                        $location_query = urlencode(trim($destination . ($country ? ', ' . $country : '')));
                    ?>
                        <div class="travel-map-section">
                            <h3>📍 Location</h3>
                            <div id="travel-map-<?php echo $travel_id; ?>" class="travel-map-container" style="width: 100%; height: 450px; border-radius: 12px; overflow: hidden; margin: 20px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"></div>
                            ...
                            <script>
                                (function() {
                                    const mapEl = document.getElementById('travel-map-<?php echo $travel_id; ?>');
                                    if (!mapEl) return;

                                    // Fetch coordinates from Nominatim (OpenStreetMap)
                                    fetch('https://nominatim.openstreetmap.org/search?format=json&q=<?php echo $location_query; ?>')
                                        .then(res => res.json())
                                        .then(data => {
                                            if (data && data.length > 0) {
                                                ...
                                                // Initialize map
                                                const map = L.map('travel-map-<?php echo $travel_id; ?>').setView([lat, lon], 10);
                                                ...
                                                marker.bindPopup('<b><?php echo esc_js($destination); ?></b><?php echo $country ? '<br>' . esc_js($country) : ''; ?>').openPopup();
                                            } else {
                                                mapEl.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; background: #f5f5f5; color: #666;"><p>📍 Location: <?php echo esc_html($destination); ?><?php echo $country ? ', ' . esc_html($country) : ''; ?></p></div>';
                                            }
                                        })
                                        .catch(err => {
