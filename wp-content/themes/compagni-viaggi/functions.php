...
function cdv_travel_meta($post_id = null) {
    ...
    <div class="travel-meta">
        ...
        <?php if (!empty($start_date) || !empty($travel_month)) : ?>
            <span class="meta-item">
                <span class="icon">📅</span>
                <?php
                if ($date_type === 'month' && !empty($travel_month)) {
                    ...
                    if ($month_timestamp !== false && $month_timestamp > 0) {
                        echo esc_html(date_i18n('F Y', $month_timestamp)) . ' <span style="color: var(--text-medium); font-size: 0.9em;">(flexible)</span>';
                    } else {
                        // Fallback parsing
                        ...
                        if (count($parts) === 2 && is_numeric($parts[0]) && is_numeric($parts[1])) {
                            ...
                            if ($month_num >= 1 && $month_num <= 12) {
                                ...
                                echo esc_html(date_i18n('F Y', $timestamp)) . ' <span style="color: var(--text-medium); font-size: 0.9em;">(flexible)</span>';
                            } else {
                    ...
                    if ($start_timestamp !== false && $start_timestamp > 0) {
                        echo esc_html(date_i18n('d/m/Y', $start_timestamp));
                        if (!empty($end_date)) {
                            $end_timestamp = @strtotime($end_date);
                            if ($end_timestamp !== false && $end_timestamp > 0) {
                                echo ' - ' . esc_html(date_i18n('d/m/Y', $end_timestamp));
                            }
                        }
                    } else {
                        // Fallback display
...
function cdv_filter_viaggi_archive($query) {
    ...
    if (!is_admin() && $query->is_main_query() && $is_viaggio_query) {
        ...
        if (!empty($_GET['date_from'])) {
            ...
            // Ensure date is valid before querying
